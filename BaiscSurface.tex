...
The ultimate goal of this part of this book is to create a surface that represents what is possible for a lifter to do. To accomplish this, the nature of the surface must first be explored. This can be done by looking at the data as well as applying already established facts about lifting.
...
\begin{itemize}
	\item Property 1.a: As intensity approaches it's maximum volume will decrease until it is near the lifters 1RM for the given effort and fatigue levels.
...
In general, fatigue will necessarily limit volume. The reasoning behind this is simple, the more fatigued a lifter is the less work they can safely do. At the extreme, fatigue will make it so no work can be done at all and hence $0$ volume can be tolerated. This is a state that should be be avoided at all costs, as it is the epitome of over training. When fatigue is at a minimum a lifter will be able to reach there highest potential in there current state. Note that fatigue cannot add to a lifters abilities, it can only diminish them. This is true regardless of effort. No matter how much effort a lifter puts into a set, they would have always been able to lift more weight had they been less fatigued.

There is one nuance to the relation between fatigue and volume. Inter-set fatigue is represented by multiplying the $s$ and $r$ values together, meaning it increases with the total number of reps performed. As this measure of fatigue increase volume should decrease. However, looking at the equation for total volume, equation \ref{eq:TotalVolumeEquaiton}, it also increases with the total number of reps preformed. This creates a contradiction where volume is both increasing and decreasing from increases in inter-set fatigue. Inter-exercise and inter-workout fatigue also rely on the total number of reps performed, but they do so on a different time scale. Inter-exercise fatigue sums fatigue on a per exercise basis, 
...
This proves that volume decreases with fatigue if and only if $\epsilon_2>0$. Just like the previous section, a similar argument can be made to prove that volume increases with decreased fatigue. This constraint will need to be applied to $\epsilon_2$, $\epsilon_3$, and $\epsilon_4$ as they are all part of $F_{tot}$ and there partial derivatives take similar forms. Now, $\epsilon_5$, $\epsilon_6$, and $\epsilon_7$ are also part of $F_{tot}$ but there partial derivatives take different forms. Because of this a separate analysis will need to be done to ensure the proper behavior. $\partial_r v$ is shown below.
...
These constraints are listed in equation \ref{eq:PotentialSurfaceEquation}. The next section should offer a break from the monotony of this section.
