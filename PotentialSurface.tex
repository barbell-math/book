...
\subsection{Volume and Intensity}
...
Given the discussion above, it is tempting to say that as weight approaches $0$ volume will linearly decrease to $0$. Again, while mathematically true, the human body does not follow a linear decrease in volume. When weight diminishes to more and more tolerable loads more and more endurance is required. Generally speaking, the human body is made for endurance. Running for long distances is one of our strengths thanks to evolution. What this means for lifting is that as weight decreases so many more sets and reps can be done that the decrease in volume from the decrease in weight is overcome. There is a limit of course, known as the lifters volume tolerance, so as weight decreases more sets and reps can be done until this limit is reached. The data displayed in figure \ref{fig:IntensityVsVolumeGraph} does not demonstrate this pattern very well, as it appears volume drops of linearly past the peak. This is due to the data being collected by a powerlifter, a sport that does not require much in the way of endurance, and skewing the data as a result. This skewing of the data will come up again in the next section, which is concerned with volume and effort.

%To summarize, the following intuitions can be stated:
%
%\begin{enumerate}
%	\item Volume increases linearly with sets and reps.
%	\item Volume decreases with weight due.
...
%    \centering
%    \begin{tabular}{c}
%        \includegraphics[width=125mm]{./images/ch3/PotentialSurface/DeadliftEffort5.png} \\
%        \includegraphics[width=125mm]{./images/ch3/PotentialSurface/DeadliftEffort10.png} \\
%    \end{tabular}
