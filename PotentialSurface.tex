...
Without any further reasoning about the behavior of $C(r)$, an equation that can represent both when fatigue deltas remain constant as well as when fatigue deltas increase is needed. A function of $r$ raised to a power $\ge 1$ will work. Technically any equation that is continually increasing, $f'>0$, could work for this scenario. The choice of using a function raised to a power is based on intuition and experience at the gym. Generally as a set goes on the increases in fatigue do not feel like they decrease. Looking at the equation below, if $\beta=1$ then a constant fatigue delta is assumed and if $\beta>1$ then an increasing fatigue delta is assumed. Depending on the value of $\beta$ that is found from fitting the model to the data it will be able to show how the body responds to fatigue on a inter-set level.
...
\begin{minipage}{\textwidth}
	\begin{equation}
		...
		= \epsilon_5 (r-1)^\beta \left( \frac{E}{10} \right)^\alpha
